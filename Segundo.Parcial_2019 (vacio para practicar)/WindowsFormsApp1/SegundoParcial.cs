...
namespace SP
{
    ...
    public partial class SegundoParcial: Form
    {
        ...
        private void btnPunto5_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                this.c_bananas.EventoPrecio += new Cajon<Banana>.Delegado(c_bananas.Guardar);
        ...
        private static string ObtenerListadoFrutas()
        {
            ...
            {//Configurar el SqlDataAdapter (y su selectCommand)
                try
                {
                    SqlCommand command = new SqlCommand("SELECT id,nombre,peso,precio FROM frutas", connection);
                    command.Connection = connection;
                    connection.Open();
                    SqlDataReader dataReader = command.ExecuteReader();
                    while (dataReader.Read())
                    {
                        stringBuilder.AppendLine($"{dataReader.GetInt32(0).ToString()} - {dataReader.GetString(1)} - {dataReader.GetDouble(2).ToString()} - {dataReader.GetDouble(3).ToString()}");
                    }
                }
                catch (Exception e)
                {
        ...
        private static bool AgregarFrutas(SegundoParcial frm)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(@"Data Source=.\SQLEXPRESS; Initial Catalog=sp_lab_II_frutas;Integrated Security = True"))
                {
                    ...
                    //ACA ESTOY TRABAJANDO DIRECTAMENTE SOBRE LA BASE DE DATOS, NO EN MEMORIA!!!
                    strConsulta = $"INSERT INTO frutas(nombre, peso, precio) VALUES('{frm._banana.Nombre}', '{frm._banana.Peso}', '0') ;";
                    strConsulta += $"INSERT INTO frutas(nombre, peso, precio) VALUES('{frm._manzana.Nombre}', '{frm._manzana.Peso}', '0') ";
                    //strConsulta += $"INSERT INTO frutas(nombre, peso, precio) VALUES('{frm._durazno.Nombre}', '{frm._durazno.Peso}', '0') ";
